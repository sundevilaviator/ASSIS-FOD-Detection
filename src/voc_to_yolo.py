...
def main() -> None:
    ...
    ap.add_argument("--voc-dir", type=Path, required=True, help="Directory containing VOC .xml annotation files (searched recursively).")
    ap.add_argument("--out", type=Path, help="Output directory for YOLO .txt label files. Required unless --list-classes-only.")
    ...
    ap.add_argument(
        ...
        help="Scan the dataset and print every distinct class name actually present, then exit "
             ...
             "before adding them to configs/fod.yaml.",
    )
    args = ap.parse_args()
    ...
    if summary.malformed_files:
        print("  Malformed files (first 10):")
